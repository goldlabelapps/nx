...
order: 9002
title: NX° Actions
description: Uberedux
icon: docs
tags: redux, state-management, dashboard
image: nx
---
...
**Note on the `typescript` document:** `initCollection` treats a Firestore document with `id === 'typescript'` as the schema definition document for that collection. It is separated from the `docs` array and stored in `crud[collection].typescript`. See the [CRUD System](/nx/crud) docs for details.
...
See the [Push Notifications](/nx/notifications) document for full details.
...
See the [PWA Support](/nx/pwa) document for full details.
