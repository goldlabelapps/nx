...
export function DropdownMenu({ item }: { item: NavItem }) {
  ...
  const triggerClasses = cn(
    ...
    isOpen
      ...
      : isSectionActive
      ? "text-[#0f172a] bg-[#0f172a]/[0.06] font-semibold"
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div className="absolute left-0 top-full pt-2 z-50 w-72 md:w-80">
                ...
                const content = (
                  <div
                    className={cn(
                      ...
                      isActive
                        ? "bg-[#0f172a]/[0.08] text-[#0f172a]"
                        : "hover:bg-[#0f172a]/[0.05]"
                    )}
                  >
                    {IconComponent && (
                      <div
                      ...
                      <div className="flex items-center gap-1.5">
                        <span
                          className={cn(
                            ...
                            isActive
                              ? "text-[#0f172a]"
                              : "text-slate-900 group-hover:text-[#0f172a]"
                          )}
                        >
                          {dropdownItem.title}
                        </span>
                        {dropdownItem.badge && (
                          <span className="rounded-full bg-white px-1.5 py-0.2 text-[10px] font-bold text-[#0f172a] border border-[#0f172a]/15">
                            {dropdownItem.badge}
                          </span>
                        )}
                        {dropdownItem.external && (
                          <ExternalLink className="h-3 w-3 text-slate-400 group-hover:text-[#0f172a]" />
                        )}
                      </div>
