...
describe("LogoContextMenu", () => {
  ...
  it("renders Template logo link", () => {
    ...
    expect(screen.getByLabelText(/goldlabel/i)).toBeInTheDocument();
  });
  ...
  it("opens context menu on right click and handles SVG copying", async () => {
    ...
    const logoLink = screen.getByLabelText(/goldlabel/i);
    const container = logoLink.closest("div")!;
    fireEvent.contextMenu(container, { clientX: 150, clientY: 50 });

    expect(screen.getByText(/Copy Template Logo as SVG/i)).toBeInTheDocument();
    expect(screen.getByText(/Template Guidelines/i)).toBeInTheDocument();

    const copyBtn = screen.getByRole("button", { name: /copy template logo as svg/i });
    await act(async () => {
  ...
  it("closes context menu when clicking outside or scrolling", () => {
    ...
    expect(screen.queryByText(/Copy Template Logo as SVG/i)).not.toBeInTheDocument();
  });
});
