---
order: 300
slug: /help
title: Help & Support
description: Reach out to support@echopay.co.uk
icon: info
tags: Support, FAQ, help
---


1. Mobile banking app is not opening to make payment? 

> Please make sure that you have the app of the bank you are attempting to pay from on your phone. Furthermore, please ensure that you are using Google Chrome or Safari and that you have them set as your default browser. We are unable to provide support for payments made on other browsers.



2. I am not being redirected to my mobile banking app?

> A few things to check before you attempt another payment would be:
Check you have selected the correct bank (i.e personal or business).
Check the browser that you are using has the correct permission to open apps. If Samsung internet is your default browser we recommend changing to Google Chrome or you can watch our YouTube video on how to change your settings for Samsung Internet.
Check that you have the banking app installed on your device and that it is up to date.

3. Lorem Ipsum

> Please make sure...

4. Dolar Immet

> Please make sure...

[echopay.co.uk/faqs](https://echopay.co.uk/faqs)

### Resources

- [Banks](/banks)  
    Statuses of all banks we support
- [Training](/training)  
Training material and demos for our services

Reach out to [support@echopay.co.uk](mailto:support@echopay.co.uk) if you can't find an answer to your question. 
